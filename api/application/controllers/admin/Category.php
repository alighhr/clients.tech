...
class Category extends API_Controller_Secure
{
	...
	*/
	public function add_post()
	{
		...
		$this->form_validation->set_rules('CategoryName', 'Category Name', 'trim|required|callback_validateCategoryName');
	...
	*/
	...
	public function validateCategoryName(){
		$Category = $this->db->query('SELECT CategoryName FROM `set_categories` WHERE `CategoryName` = "'. $this->Post['CategoryName'] .'"')->row()->CategoryName;
		if(!empty($Category))
		{
		    $this->form_validation->set_message('validateCategoryName', 'You have already created this category');
         	return FALSE;
		}else{
			return TRUE;
		}
