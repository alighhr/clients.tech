    ...
    public function editTeam_post() {
        ...
        if (!empty($this->Post['MediaGUIDs'])) {
            ...
            foreach ($MediaGUIDsArray as $MediaGUID) {
                ...
                $MediaData = $this->Media_model->getMedia('E.EntityGUID MediaGUID,M.MediaName', array("SectionID" => "TeamFlag", "MediaID" => $EntityData['MediaID']), FALSE);
            }
        }
        $this->Football_Sports_model->updateTeamFlag($this->TeamID, array('TeamName' => $this->Post['TeamName'], 'TeamFlag' => @$MediaData['MediaName']));
        /* check for media present - associate media with this Post - ends */


        $TeamData = $this->Football_Sports_model->getTeams('TeamName,TeamNameShort,TeamFlag', array("TeamID" => $this->TeamID));
