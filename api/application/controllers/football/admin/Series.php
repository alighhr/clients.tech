...
class Series extends API_Controller_Secure {
    ...
    public function getAuctionDraftPlayers_post() {
        ...
        $this->form_validation->validation($this);  /* Run validation */
        /* Get Players Data */
        $playersData = $this->Football_Sports_model->getAuctionDraftPlayers(@$this->Post['Params'], array_merge($this->Post, array('TeamID' => @$this->TeamID, 'MatchID' => @$this->MatchID, 'SeriesID' => @$this->SeriesID, 'UserID' => @$this->SessionUserID)), TRUE, @$this->Post['PageNo'], @$this->Post['PageSize']);
    ...
    public function getFilterData_post() {
        ...
        $SeriesData = $this->Football_Sports_model->getSeries(@$this->Post['Params'], array());
    ...
    public function changeStatus_post() {
        ...
        /* if ($this->Post['Status'] == 'Inactive') {
          $MatchesData = $this->Football_Sports_model->getMatches('MatchID', array('SeriesID' => $this->SeriesID, 'StatusID' => 1), TRUE, 0);
          ...
          $ContestQuery = 'SELECT ContestID,EntryFee FROM football_sports_contest WHERE MatchID =' . $value['MatchID'];
          $ContestData = $this->db->query($ContestQuery)->result_array();
          if (!empty($ContestData)) {
          foreach ($ContestData as $Contest) {
          $this->Football_Contest_model->cancelContest($Contest, $this->SessionUserID, $Contest['ContestID']);
          }
          }
          }
          }
          ...
          $MatchesData = $this->Football_Sports_model->getMatches('MatchID', array('SeriesID' => $this->SeriesID, 'StatusID' => 3, 'MatchStartDateTimeComplete' => date('Y-m-d H:i')), TRUE, 0);
          if (!empty($MatchesData['Data']['Records'])) {
          foreach ($MatchesData['Data']['Records'] as $value) {
    ...
    public function getSeriesDetails_post() {
        ...
        /* Get Contests Data */
        $SeriesData = $this->Football_Sports_model->getSeries(@$this->Post['Params'], array_merge($this->Post, array('SeriesID' => $this->SeriesID)), FALSE, 0);
        if (!empty($SeriesData)) {
    ...
    public function updateRounds_post() {
        ...
        $updateData = $this->Football_Sports_model->updateRounds($this->Post);
        if ($updateData) {
