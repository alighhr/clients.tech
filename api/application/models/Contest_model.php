...
class Contest_model extends CI_Model {
    ...
    function getContests($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        if (!empty($Where['ContestFull']) && $Where['ContestFull'] == 'No') {
            ...
            $this->db->having("IF(C.UnfilledWinningPercent = 'Fixed',TotalJoined != C.ContestSize,TotalJoined >= C.ContestSize)",NULL, FALSE);
        }
            ...
            if ($multiRecords) {
                ...
                if (in_array('UserTeamPlayers', $Params)) {
                    foreach ($Return['Data']['Records'] as $key => $value) {
                        ...
                        $UserTeamPlayers = $this->getUserTeamPlayers('PlayerID,IsPlaying,PlayerSalary,PlayerSalaryCredit,PlayerPosition,PlayerBattingStyle,PlayerSelectedPercent,PlayerBowlingStyle,PlayerName,PlayerPic,PlayerCountry,PlayerRole,Points,TeamGUID,TotalPoints,TotalPointCredits', array('UserTeamID' => $value['UserTeamID']));
                ...
                if (in_array('UserTeamPlayers', $Params)) {
                    ...
                    $UserTeamPlayers = $this->getUserTeamPlayers('PlayerID,IsPlaying,PlayerSelectedPercent,PlayerSalary,PlayerSalaryCredit,TeamGUID,PlayerPosition,PlayerBattingStyle,PlayerBowlingStyle,PlayerName,PlayerPic,PlayerCountry,PlayerRole,Points,TotalPoints,TotalPointCredits', array('UserTeamID' => $Where['UserTeamID']));
    ...
    function getVirtualTeamPlayerMatchWise($MatchID, $DummyUserPercentage, $ContestID) {
        ...
        $this->db->where('EXISTS (select UserID from tbl_users U where U.UserID = SUT.UserID AND U.UserTypeID=3) AND NOT EXISTS (select UserTeamID from sports_contest_join SCJ where SCJ.UserID = `SUT`.`UserID` AND SCJ.UserTeamID = `SUT`.`UserTeamID` AND `SCJ`.`MatchID` = `SUT`.`MatchID` AND SCJ.MatchID = "' . $MatchID . '" AND SCJ.ContestID = "' . $ContestID . '")');
    ...
    function getJoinedContestsUsersMongoDB($Where = array(), $PageNo = 1, $PageSize = 15) {
        ...
        if (count($JoinedContestsUsersAll) > 0) {
    ...
    function getAccountReport($Where = array(), $SeriesID = '') {
        ...
        $this->db->select('M.MatchID,S.SeriesID,S.SeriesStartDate,S.SeriesEndDate,S.SeriesName,CONCAT(TL.TeamName," Vs ",TV.TeamName) as MatchName,DATE_FORMAT(CONVERT_TZ(M.MatchStartDateTime,"+00:00","' . DEFAULT_TIMEZONE . '"), "' . DATE_FORMAT . '") MatchStartDateTime');
        $this->db->from('sports_matches M,tbl_entity E,sports_series S,sports_teams TL, sports_teams TV');
        $this->db->where("E.EntityID", "M.MatchID", FALSE);
        $this->db->where("S.SeriesID", "M.SeriesID", FALSE);
        $this->db->where("M.TeamIDLocal", "TL.TeamID", FALSE);
