...
class Football_PreContest_model extends CI_Model {
    ...
    function addContest($Input = array(), $SessionUserID, $StatusID = 1) {
        ...
        $this->db->insert('football_sports_pre_contest', $InsertData);
    ...
    function updateContest($Input = array(), $SessionUserID, $PreContestID, $StatusID = 1) {
        ...
        $this->db->limit(1);
        $this->db->update('football_sports_pre_contest', $UpdateData);

        $Contests = $this->db->query("SELECT
                                        C.ContestID
                                    FROM
                                        football_sports_contest C,tbl_entity E
                                    WHERE 
        ...
        if (!empty($Contests)) {
            foreach ($Contests as $Rows) {
                ...
                $this->db->limit(1);
                $this->db->update('football_sports_contest', $UpdateContest);
            }
        }
    }
    ...
    function deleteContest($SessionUserID, $PreContestID) {
        ...
        $this->db->limit(1);
        $this->db->delete('football_sports_pre_contest');
    }
        ...
        $Contests = $this->db->query("SELECT
                                    ...
                                    WHERE E.EntityID=C.ContestID AND NOT EXISTS
                                        ...
                                        SELECT
                                            1
                                        FROM
                                            football_sports_contest_join
                                        WHERE
                                            football_sports_contest_join.ContestID = C.ContestID
    ...
    function getPreContest($Field = '', $Where = array(), $multiRecords = FALSE, $PageNo = 1, $PageSize = 15) {
        ...
        $this->db->from('football_sports_pre_contest C');
            ...
            if ($multiRecords) {
                ...
                if (!empty($Where['MatchID'])) {
                    $Record['Statics'] = $this->db->query('SELECT (SELECT COUNT(*) AS `NormalContest` FROM `football_sports_contest` C, `tbl_entity` E WHERE C.ContestID = E.EntityID AND E.StatusID IN (1,2,5) AND C.MatchID = "' . $Where['MatchID'] . '" AND C.ContestType="Normal" AND C.ContestFormat="League" AND C.ContestSize != (SELECT COUNT(*) from football_sports_contest_join where football_sports_contest_join.ContestID = C.ContestID)
                            ...
                            ( SELECT COUNT(*) AS `ReverseContest` FROM `football_sports_contest` C, `tbl_entity` E WHERE C.ContestID = E.EntityID AND E.StatusID IN(1,2,5) AND C.MatchID = "' . $Where['MatchID'] . '" AND C.ContestType="Reverse" AND C.ContestFormat="League" AND C.ContestSize != (SELECT COUNT(*) from football_sports_contest_join where sports_contest_join.ContestID = C.ContestID)
                            )as ReverseContest,(
                            SELECT COUNT(*) AS `JoinedContest` FROM `football_sports_contest_join` J, `sports_contest` C, `tbl_entity` E WHERE C.ContestID = J.ContestID AND C.ContestID = E.EntityID AND E.StatusID != 3 AND J.UserID = "' . @$Where['SessionUserID'] . '" AND C.MatchID = "' . $Where['MatchID'] . '" 
                            )as JoinedContest,( 
        ...
        if (!empty($Where['MatchID'])) {
            $Return['Data']['Statics'] = $this->db->query('SELECT (SELECT COUNT(*) AS `NormalContest` FROM `football_sports_contest` C, `tbl_entity` E WHERE C.ContestID = E.EntityID AND E.StatusID IN (1,2,5) AND C.MatchID = "' . $Where['MatchID'] . '" AND C.ContestType="Normal" AND C.ContestFormat="League" AND C.ContestSize != (SELECT COUNT(*) from football_sports_contest_join where football_sports_contest_join.ContestID = C.ContestID)
                    ...
                    SELECT COUNT(*) AS `JoinedContest` FROM `football_sports_contest_join` J, `football_sports_contest` C, `tbl_entity` E WHERE C.ContestID = J.ContestID AND C.ContestID = E.EntityID AND E.StatusID != 3 AND J.UserID = "' . @$Where['SessionUserID'] . '" AND C.MatchID = "' . $Where['MatchID'] . '" 
                    )as JoinedContest,( 
                    SELECT COUNT(*) AS `TotalTeams` FROM `football_sports_users_teams`WHERE UserID = "' . @$Where['SessionUserID'] . '" AND MatchID = "' . $Where['MatchID'] . '"
                ) as TotalTeams,(SELECT COUNT(*) AS `H2HContest` FROM `football_sports_contest` C, `tbl_entity` E, `football_sports_contest_join` CJ WHERE C.ContestID = E.EntityID AND E.StatusID IN (1,2,5) AND C.MatchID = "' . $Where['MatchID'] . '" AND C.ContestFormat="Head to Head" AND E.StatusID = 1 AND C.ContestID = CJ.ContestID AND C.ContestSize != (SELECT COUNT(*) from football_sports_contest_join where football_sports_contest_join.ContestID = C.ContestID )) as H2HContests')->row();
        }
    ...
    function createPreContest($InsertID = '') {
        ...
        if (!empty($InsertID)) {
            $ContestData = $this->db->query('SELECT * FROM football_sports_pre_contest WHERE PreContestID = ' . $InsertID);
        } else {
            $ContestData = $this->db->query('SELECT * FROM football_sports_pre_contest');
        }

        if ($ContestData->num_rows() > 0) {

            /* Get matches of next 5 days */
            $DateTime = date('Y-m-d H:i', strtotime('+3 days', strtotime(date('Y-m-d H:i'))));
            $MatchesData = $this->Football_Sports_model->getMatches('SeriesID,MatchID', array('OrderBy' => 'MatchStartDateTime', 'Sequence' => 'ASC', 'StatusID' => 1, 'MatchStartDateTime' => $DateTime), TRUE, 1, 50);
            ...
            foreach ($ContestData->result_array() as $Res) {
                ...
                foreach ($MatchesData['Data']['Records'] as $Record) {
                    $GetContest = $this->db->query('SELECT * FROM football_sports_contest C, tbl_entity E WHERE E.EntityID=C.ContestID 
                        AND C.PreContestID = ' . $Res['PreContestID'] . ' AND C.MatchID = ' . $Record['MatchID'] . ' AND E.StatusID=1');
                    if ($GetContest->num_rows() == 0) {
                        $this->Football_Contest_model->addContest($FieldArray, '125', $Record['MatchID'], $Record['SeriesID']);
                    }
                }
            }
        }
