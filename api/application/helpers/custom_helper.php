...
function sendPushMessage($UserID, $Title, $Message, $Data = array()) {
    ...
    if ($Query->num_rows() > 0) {
        foreach ($Query->result_array() as $Notifications) {
            if ($Notifications['DeviceTypeID'] == 2) { /* I phone */
                pushNotificationIphone($Notifications['DeviceToken'], $Notifications['UserTypeID'], $Title, $Message, $Data);
            } elseif ($Notifications['DeviceTypeID'] == 3) { /* android */
                pushNotificationAndroid($Notifications['DeviceToken'], $Notifications['UserTypeID'], $Title, $Message, $Data);
            }
        }
    }
}
...
function pushNotificationAndroid($DeviceIDs, $UserTypeID, $Title, $Message, $Data = array()) {
    //API URL of FCM
    $post_fcm = array();
...
function pushNotificationIphone($DeviceToken = '', $UserTypeID, $Title, $Message = '', $Data = array()) {
    //API URL of FCM
    $post_fcm = array();
...
function mongoDBConnection() {
    ...
    switch (ENVIRONMENT) {
        case 'local':
            ...
            break;
        case 'testing':
            $Obj->ClientObj = new MongoDB\Client("mongodb://test:test2019@localhost:27017");
            break;
        case 'demo':
            $Obj->ClientObj = new MongoDB\Client("mongodb://localhost:58017");
            break;
            default:
            $Obj->ClientObj = new MongoDB\Client("mongodb://username:password@localhost:27017");
            break;
    }
    $rc = $Obj->ClientObj->getWriteConcern();
    $Obj->fantasydb = $Obj->ClientObj->super6;
}
