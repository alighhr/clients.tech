app.controller('PageController', function ($scope, $http, $timeout, $rootScope) {
    ...
    $scope.getFilterData = function ()
    {
        ...
        var data = 'SessionKey=' + SessionKey + '&Params=SeriesID,RoundID,SeriesName,SeriesMatchStartDate&IsPlayRounds=Yes&StatusID=2&AuctionDraftIsPlayed=Yes&AuctionDraftStatusID=1&' + $('#filterPanel form').serialize();

        $http.post(API_URL + 'admin/matches/getFilterDataRounds', data, contentType).then(function (response) {
    ...
    $scope.getRoundPlayers = function (RoundID)
    {
        ...
        $http.post(API_URL + 'admin/matches/getRoundPlayers', data, contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200 && response.Data) {
                /* success case */
                $scope.filterDataRoundPlayers = response.Data;
                
                var I = $scope.filterData.SeiresData.map(e =>{ return e.RoundID;}).indexOf(RoundID); 
                if(I != -1){
                    $('#MatchStartDate').val($scope.filterData.SeiresData[I].SeriesMatchStartDate)
                }
            }
        });
        ...
        $http.post(API_URL + 'admin/auctionDrafts/add', data, contentType).then(function (response) {
            ...
            if (response.ResponseCode == 200) { /* success case */
               ...
               // $('.modal-header .close').click();
                window.location.href = 'auctionDrafts';
            } else {
    ...
    $scope.loadFormStatus = function (Position, ContestGUID)
    {
        ...
        $scope.data.pageLoading = true;
        $http.post(API_URL + 'auctionDrafts/getContest', 'SessionKey=' + SessionKey + '&ContestGUID=' + ContestGUID + '&Params=ContestName,ContestType,Status,StatusID', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.pageLoading = false;
                $scope.formData = response.Data
