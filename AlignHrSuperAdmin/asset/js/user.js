app.controller('PageController', function ($scope, $http, $timeout) {
    ...
    $scope.loadFormEdit = function (Position, UserGUID) {
        ...
        $http.post(API_URL + 'users/getProfile', 'SessionKey=' + SessionKey + '&UserGUID=' + UserGUID + '&Params=isWithdrawal,IsPrivacyNameDisplay,Status,ProfilePic,MediaPAN,MediaBANK,RegisteredOn,EmailForChange,EmailStatus,PhoneNumberForChange,PhoneStatus,LastLoginDate,UserTypeName, FullName, Email, Username, ProfilePic, Gender, BirthDate, PhoneNumber, ReferredCount,StatusID,BusinessName,Domain,PaymentMode,SubscriptionType,StartDate,EndDate,Address', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
    ...
    $scope.editData = function () {
        ...
        $http.post(API_URL + 'admin/users/edit', data, contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                alertify.success(response.Message);
