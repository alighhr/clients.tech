app.controller('PageController', function ($scope, $http, $timeout, $rootScope) {
    ...
    $scope.getList = function (Type = '')
    {
        ...
        $http.post(API_URL_FOOTBALL + 'admin/preContest/getPreContest', data, contentType).then(function (response) {
            var response = response.data;
    ...
    $scope.loadFormEdit = function (Position, PreContestID)
    {
        ...
        $http.post(API_URL_FOOTBALL + 'admin/preContest/getPreContest', 'SessionKey=' + SessionKey + '&PreContestID=' + PreContestID + '&Params=WinningRatio,WinUpTo,UnfilledWinningPercent,SmartPool,IsVirtualUserJoined,VirtualUserJoinedPercentage,AdminPercent,Privacy,IsPaid,WinningAmount,ContestSize,EntryFee,NoOfWinners,EntryType,CustomizeWinning,ContestType,CashBonusContribution,UserJoinLimit,ContestFormat,IsConfirm,ShowJoinedContest,IsAutoCreate',contentType).then(function (response) {
            var response = response.data;
    ...
    $scope.deleteContest = function (Position, PreContestID)
    {
        ...
        if(confirm('Are you sure, want to delete this Contest ?')){
            ...
            $http.post(API_URL_FOOTBALL+'admin/preContest/delete', 'SessionKey='+SessionKey+'&PreContestID='+PreContestID, contentType).then(function(response) {
                var response = response.data;
                if(response.ResponseCode==200){ /* success case */
    ...
    $scope.deleteAllUpcomingContest = function (Position, PreContestID)
    {
        ...
        if (confirm('Are you sure, want to delete upcoming Pre contest ?')) {
            $scope.addDataLoading = true;
            $http.post(API_URL_FOOTBALL + 'admin/preContest/deleteUpcomingPreContest', 'SessionKey=' + SessionKey + '&PreContestID=' + PreContestID, contentType).then(function (response) {
                var response = response.data;
                if (response.ResponseCode == 200) { /* success case */
                    alertify.success(response.Message);
                    setTimeout(function () {
    ...
    $scope.getCategoriesList = function ()
    {
        ...
        $http.post(API_URL_FOOTBALL + 'category/getCategories', data, contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.totalRecords = response.Data.TotalRecords;
                for (var i in response.Data.Records) {
    ...
    $scope.addDataPreContest = function ()
    {
        ...
        $http.post(API_URL_FOOTBALL + 'admin/preContest/add', PostData, contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                alertify.success(response.Message);
                $scope.applyFilter();
    ...
    $scope.editData = function ()
    {
       ...
       $http.post(API_URL_FOOTBALL + 'admin/preContest/edit', $.param(inputData), contentType).then(function (response) {
    ...
    $scope.loadFormStatus = function (Position, ContestGUID)
    {
        ...
        $scope.data.pageLoading = true;
        $http.post(API_URL_FOOTBALL + 'contest/getContest', 'SessionKey=' + SessionKey + '&ContestGUID=' + ContestGUID + '&Params=ContestName,ContestType,Status,StatusID', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.pageLoading = false;
                $scope.formData = response.Data
    ...
    $scope.loadContestJoinedUser = function (Position, ContestGUID)
    {
        ...
        $scope.data.pageLoading = true;
        $http.post(API_URL_FOOTBALL + 'contest/getJoinedContestsUsers', 'SessionKey=' + SessionKey + '&ContestGUID=' + ContestGUID + '&Params=UserTeamName,TotalPoints,UserWinningAmount,FirstName,Username,UserGUID,UserTeamPlayers,UserTeamID,UserRank', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.pageLoading = false;
                $scope.formData = response.Data;
    ...
    $scope.editStatus = function (Status, contestGUID)
    {
        if (Status == 'Cancelled') {
            if(confirm('Are you sure, want to cancel contest ?')){
                ...
                $http.post(API_URL_FOOTBALL + 'admin/contest/cancel', req, contentType).then(function (response) {
                    var response = response.data;
                    if (response.ResponseCode == 200) { /* success case */
                        $scope.editDataLoading = true;
                        var data = 'SessionKey=' + SessionKey + '&' + $("form[name='update_form']").serialize();
                        $http.post(API_URL_FOOTBALL + 'admin/contest/changeStatus', data, contentType).then(function (response) {
                            var response = response.data;
                            if (response.ResponseCode == 200) { /* success case */
                                alertify.success(response.Message);
                                $scope.data.dataList[$scope.data.Position] = response.Data;
