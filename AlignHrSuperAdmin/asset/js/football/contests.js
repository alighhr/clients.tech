app.controller('PageController', function ($scope, $http, $timeout, $rootScope) {
    ...
    $scope.addData = function ()
    {
        ...
        $http.post(API_URL_FOOTBALL + 'admin/contest/add', data, contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
    ...
    $scope.editData = function ()
    {
        ...
        $http.post(API_URL_FOOTBALL + 'admin/contest/edit', data, contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                alertify.success(response.Message);
    ...
    $scope.loadContestJoinedUser = function (Position, ContestGUID)
    {
        ...
        $scope.data.pageLoading = true;
        $http.post(API_URL_FOOTBALL + 'contest/getJoinedContestsUsers', 'SessionKey=' + SessionKey + '&ContestGUID=' + ContestGUID + '&Params=UserTeamName,Email,PhoneNumber,TotalPoints,UserWinningAmount,FirstName,Username,UserGUID,UserTeamPlayers,UserTeamID,UserRank', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.pageLoading = false;
                $scope.formData = response.Data;
