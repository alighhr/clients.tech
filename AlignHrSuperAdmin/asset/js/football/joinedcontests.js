app.controller('PageController', function ($scope, $http,$timeout){
    ...
    $scope.getList = function ()
    {
        ...
        $http.post(API_URL_FOOTBALL+'admin/contest/getUserJoinedContests', data, contentType).then(function(response) {
        var response = response.data;
    ...
    $scope.loadWinnersForm = function(Position, ContestGUID) {
        ...
        $http.post(API_URL_FOOTBALL + 'contest/getContest', 'SessionKey=' + SessionKey + '&ContestGUID=' + ContestGUID + '&Params=CustomWinnings', contentType).then(function(response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
