app.controller('PageController', function ($scope, $http,$timeout){
    ...
    /*load edit form*/
    $scope.loadFormEdit = function (Position, FeatureGUID)
    {
        $scope.data.Position = Position;
        $scope.templateURLEdit = PATH_TEMPLATE+module+'/edit_form.htm?'+Math.random();
        $scope.data.pageLoading = true;
        $http.post(API_URL+'feature/getFeature', 'SessionKey='+SessionKey+'&FeatureGUID='+FeatureGUID, contentType).then(function(response) {
            var response = response.data;
    ...
    /*load delete form*/
    $scope.loadFormDelete = function (Position, FeatureGUID)
    {
        $scope.data.Position = Position;
    ...
    $scope.addData = function ()
    {
        ...
        $http.post(API_URL+'admin/feature/add', data, contentType).then(function(response) {
            var response = response.data;
            if(response.ResponseCode==200){ /* success case */               
    ...
    $scope.editData = function ()
    {
        ...
        $http.post(API_URL+'admin/feature/editFeature', data, contentType).then(function(response) {
            var response = response.data;
            if(response.ResponseCode==200){ /* success case */               
                alertify.success(response.Message);
